<?php
	...
	$agent_id	=	get_the_ID();
	$author_id = get_post_field( 'post_author', $agent_id );
	//get properties
	$image_id = '';
	...
	(
	 ...
	 $comments = get_comments(array('post_id' => $agent_id, 'orderby' => 'post_date' ,'order' => 'DESC', 'post_type' => 'property-agents',  'status'  => 'approve','parent'=>0, 'number' => $no_off));
$badge_clr = $auth_status = '';	
$allowed_html = propertya_allowed_html(); 
if(get_post_meta($agent_id, 'agent_badge_txt', true ) !="" && get_post_meta($agent_id, 'agent_badge_clr', true )!="")
{
...
$is_featured = '';
if(get_post_meta($agent_id, 'agent_is_featured', true ) !="" && get_post_meta($agent_id, 'agent_is_featured', true )=="1")
{
	$is_featured = '<div class="corner-ribbon top-right" data-toggle="tooltip" data-placement="top" data-original-title="'.esc_attr__('Featured','propertya').'"><i class="fas fa-fire"></i></div>';
}
$my_type = '';
...
$ratings = array();
$ratings = propertya_reviews_stats_average($agent_id,$type,$reference);
$author_id = $cover_img = '';
$cover_img = propertya_placeholder_cover($agent_id);
$search_page_link = propertya_framework_get_link('page-property-search.php');
$author_id = get_post_field( 'post_author', $agent_id );
$auth_name = basename(get_permalink($agent_id));
?>
    ...
    <div class="row page-section ag-short-detail">
      <div class="col-sm-12 col-md-12 col-lg-3 col-xl-3 col-12">
        ...
        <div class="image-container">
           ...
           ?>
         <img src="<?php echo esc_url(propertya_placeholder_images('agent',$agent_id,'large')); ?>" class="img-fluid img-full" alt="<?php echo esc_attr(get_post_meta($image_id, '_wp_attachment_image_alt', TRUE)); ?>"/>
        </div> 
    </div>
      </div>
      <div class="col-lg-7 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="d-flex flex-row"> 
          ...
          <div class="list-heading">
            <h1><?php echo get_the_title($agent_id); ?></h1>
          </div>
          <div class="m-listing-addr">
            <?php if(get_post_meta($agent_id, 'agent_street_addr', true )!="") { ?>
            <i class="fas fa-location-arrow clr-yal"></i> <?php echo esc_html(get_post_meta($agent_id, 'agent_street_addr', true )); ?> 
            <?php } ?>
            <?php if(get_post_meta($agent_id, 'agent_latt', true )!="" && get_post_meta($agent_id, 'agent_long', true )!="") { ?>
            <a class="m-listing-map" href="https://www.google.com/maps?daddr=<?php echo esc_attr(get_post_meta($agent_id, 'agent_latt', true )); ?>,<?php echo esc_attr(get_post_meta($agent_id, 'agent_long', true )); ?>" target="_blank"><?php echo esc_html__('Get Directions','propertya');?></a>
            <?php } ?>
          </div>
          <div class="list-meta">
            <ul>
            <?php if(get_post_meta($agent_id, 'agent_mobile', true )!="") { ?>

	         <li class="single-d-mobile">
	  <span class="list-posted-date">
	    <i class="fas fa-phone-alt clr-yal"></i>
	    <a class="click-reveal phonenumber" href="tel:<?php echo esc_attr(get_post_meta($agent_id, 'agent_mobile', true )); ?>" data-original-title="<?php echo esc_html(get_post_meta($agent_id, 'agent_mobile', true )); ?>">
	      <?php echo esc_html(get_post_meta($agent_id, 'agent_mobile', true )); ?>
	    </a>
	  </span>
	</li>
            ...
            <?php } ?>  
             ...
             <?php } ?>
             <?php
			 if(intval(get_post_meta($agent_id, 'prop_'.$reference.'_singletotal_views', true)!="")) { ?>
              <li class="list-meta-with-icons single-d-views"><?php echo esc_html__('Views','propertya'); ?> : <?php echo propertya_number_format_short(get_post_meta($agent_id, 'prop_'.$reference.'_singletotal_views', true));?></li>

             <?php } ?> 
 			<li class="list-meta-with-icons single-d-views"><?php echo esc_html__('Allowed Listings','propertya'); ?> : <?php 
        ...
        <div class="col-sm-12 col-md-12 col-lg-9 col-xl-9 col-12">
          <div class="menu-des d-flex">
            <ul class="list-unstyled ssg">
             <?php if(!empty(get_the_content($agent_id)) && isset( $propertya_options['prop_agent_detail_sections']['enabled']) &&  !empty($propertya_options['prop_agent_detail_sections']['enabled']['over'])) { ?>
             ...
             <?php } ?>  
             <?php if(!empty($propes) && is_array($propes) && count($propes) > 0 && isset( $propertya_options['prop_agent_detail_sections']['enabled']) &&  !empty($propertya_options['prop_agent_detail_sections']['enabled']['props']))  { ?>
              ...
              <?php } ?>
              <?php if(isset( $propertya_options['prop_agent_detail_sections']['enabled']) &&  !empty($propertya_options['prop_agent_detail_sections']['enabled']['views'])) { ?>
              <li class="list-inline-item"><a href="#p-views" class="cl-class"><?php echo propertya_strings('prop_settings_detail_menuviews'); ?></a></li>
               <?php } ?>
               <?php if(!empty($comments) && is_array($comments) && count($comments) > 0 && isset( $propertya_options['prop_agent_detail_sections']['enabled']) &&  !empty($propertya_options['prop_agent_detail_sections']['enabled']['review'])) { ?>
              <li class="list-inline-item"><a href="#p-reviews" class="cl-class"><?php echo propertya_strings('prop_settings_detail_menureviews'); ?></a></li>
               <?php } ?>
               <?php if(isset( $propertya_options['prop_agent_detail_sections']['enabled']) &&  !empty($propertya_options['prop_agent_detail_sections']['enabled']['write_review'])) { ?> 
...
<section class="custom-padding">
    <div class="container">
        <div class="row">
             ...
             <?php
				$layout = '';
				$layout = isset( $propertya_options['prop_agent_detail_sections']['enabled']) ? $propertya_options['prop_agent_detail_sections']['enabled'] : '';
				 if ($layout): foreach ($layout as $key => $value)
				 {
						...
						switch ($key)
						{
							case 'over': get_template_part('template-parts/agent-detial/overview/overview');
							break;
							
							case 'props': get_template_part('template-parts/agent-detial/listings/properties');
							break;
							
							case 'views': get_template_part('template-parts/agent-detial/views/views');
							break;
             ...
             <div class="col-xl-4 col-lg-4 col-sm-12 col-md-12 col-12">
				...
				$sidebar_layout = isset( $propertya_options['prop_agent_detail_sidebar_agency']['enabled']) ? $propertya_options['prop_agent_detail_sidebar_agency']['enabled'] : '';
