	...
	<div data-title="<?php echo esc_attr( $package_name ); ?>">
			...
			<?php if ( is_cart() ) : ?>
				<p class="woocommerce-shipping-destination font-size-sm">
					...
					if ( $formatted_destination ) {
						...
						printf( esc_html__( 'Shipping to %s.', 'propertya' ) . ' ', '<span class="font-weight-medium">' . esc_html( $formatted_destination ) . '</span>' );
						$calculator_text = esc_html__( 'Change address', 'propertya' );
					} else {
						echo wp_kses_post( apply_filters( 'woocommerce_shipping_estimate_html', esc_html__( 'Shipping options will be updated during checkout.', 'propertya' ) ) );
					}
					?>
				</p>
			<?php endif; ?>
		...
		elseif ( ! $has_calculated_shipping || ! $formatted_destination ) : ?>
			<p class="font-size-sm">
				<?php echo wp_kses_post( apply_filters( 'woocommerce_shipping_may_be_available_html', esc_html__( 'Enter your address to view shipping options.', 'propertya' ) ) ); ?>
			</p>
		<?php elseif ( ! is_cart() ) : ?>
			<p class="font-size-sm">
				<?php echo wp_kses_post( apply_filters( 'woocommerce_no_shipping_available_html', esc_html__( 'There are no shipping options available. Please ensure that your address has been entered correctly, or contact us if you need any help.', 'propertya' ) ) ); ?>
			</p>
		<?php else : ?>
			<p class="font-size-sm">
				<?php
				// Translators: $s shipping destination.
				echo wp_kses_post( apply_filters( 'woocommerce_cart_no_shipping_available_html', sprintf( esc_html__( 'No shipping options were found for %s.', 'propertya' ) . ' ', '<span class="font-weight-medium">' . esc_html( $formatted_destination ) . '</span>' ) ) );
				?>
			</p>
			<?php
			$calculator_text = esc_html__( 'Enter a different address', 'propertya' );
		endif;
		?>
