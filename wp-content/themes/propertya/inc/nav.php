<?php
if (!function_exists('propertya_mycustom_menu')) {
    function propertya_mycustom_menu($item_id, $item, $menu_level = 1, $show_indicator = true)
    {
        $item_data = array();
        $arr = array();
        $angle =$angleAerrow = '';
...
if (!function_exists('propertya_main_menu')) {
    function propertya_main_menu($theme_location)
    {
        $menu_html = '';
            ...
            if (isset($menu->term_id)) {
                ...
                foreach ($menu_items as $item) {
                    if ($item->menu_item_parent == 0) {
                        $level1 = 1;
                        $menuItems1 = propertya_mycustom_menu($item->ID, $item, 1);
						$openSide = '';
                        ...
                        foreach ($menu_items as $sub_item) {
                            if ($item->ID == $sub_item->menu_item_parent) {
                                ...
                                if ($mainMenuClass == 'drop-down' || $mainMenuClass == 'drop-down-multilevel') {
                                    $menuItems2 = propertya_mycustom_menu($sub_item->ID, $sub_item, 2);
                                    if ($level1 == 1) {
                                    ...
                                    foreach ($menu_items as $sub_sub_item) {
                                        if ($sub_item->ID == $sub_sub_item->menu_item_parent) {
                                            ...
                                            $menuItems3 = propertya_mycustom_menu($sub_sub_item->ID, $sub_sub_item, 3, $show_indicator);
                                            $mega_menu_html .= '<li>';
                ...
                $menu_html .= '<li><a href="' . esc_url(home_url( '/' )) . '">' . esc_html__("Home", "propertya") . '</a></li>';
            }
        } else {
            $menu_html .= '<li><a href="' . esc_url(home_url( '/' )) . '">' . esc_html__("Home", "propertya") . '</a></li>';
        }
        echo propertya_navcustom_callback($menu_html, $theme_location);
    }

}


add_action('init', array('propertya_Nav_Menu_Item_Custom_Fields', 'setup'));
if (!class_exists('propertya_Nav_Menu_Item_Custom_Fields')) {

    class propertya_Nav_Menu_Item_Custom_Fields
    {
        ...
        static function setup()
        {
            if (!is_admin())
                return;
            $new_fields = apply_filters('propertya_menu_item_additional_fields', array());
            if (empty($new_fields))
                return;

            self::$options['fields'] = self::get_fields_schema($new_fields);
            add_filter('wp_edit_nav_menu_walker', function () {
                return 'propertya_Walker_Nav_Menu_Edit';
            });
            ...
            foreach (self::$options['fields'] as $field) {
                ...
                if ($field['input_type'] == 'only_text') {
                    ...
                    if ($field['name'] == 'is_megamenu_class') {
                        ...
                        $vals = array(esc_html__("Select Option", "propertya") => "", esc_html__("Simple Menu", "propertya") => "simple-menu", esc_html__("Mega Menu", "propertya") => "mega-menu");
                    ...
                    if ($field['name'] == 'is_megamenu') {
                        ...
                        for ($size = 12; $size >= 1; $size--) {
                            ...
                            $ar .= '<option value="col-12 col-sm-12 col-md-' . esc_attr($size) . ' col-lg-' . esc_attr($size) . '" ' . $selected . '>' . esc_html__("Column size ", "propertya") . $size . '</option>';
                        }
                    }
                    ...
                    if ($field['name'] == 'menu_open_side') {
                        $savedValue = get_post_meta($item->ID, self::get_menu_item_postmeta_key($field['name']), true);
                        $vals = array(esc_html__("Select Option", "propertya") => "", esc_html__("Left to Right", "propertya") => "submenu-left", esc_html__("Right to Left", "propertya") => "submenu-right");
                        $ar = '';
                        foreach ($vals as $val => $name) {
                            $selected = ($savedValue == $name) ? 'selected="selected"' : '';
                            $ar .= '<option value="' . esc_attr($name) . '" ' . $selected . '>' . esc_html($val) . '</option>';
                    ...
                    if ($field['name'] == 'menu_simple_offset') {
                        $savedValue1 = get_post_meta($item->ID, self::get_menu_item_postmeta_key($field['name']), true);
                        $ar = '';
                        $ar .= '<option value="">' . esc_html__("Select Option - (No Offset)", "propertya") . '</option>';
                        for ($size = 12; $size >= 1; $size--) {
                            $selected = ('offset-' . $size == $savedValue1) ? 'selected="selected"' : '';
                            $ar .= '<option value="offset-' . esc_attr($size) . '" ' . $selected . '>' . esc_html__("Column Offset ", "propertya") . $size . '</option>';
                        }
                    }
                    ...
                    if ($field['name'] == 'menu_vertical_offset') {
                        ...
                        for ($size = 12; $size >= 1; $size--) {
                            ...
                            $ar .= '<option value="offset-' . esc_attr($size) . '-vertical" ' . $selected . '>' . esc_html__("Vertical Offset", "propertya") . $size . '</option>';
                        }
                    }
...
if (!class_exists('propertya_Walker_Nav_Menu_Edit')) {

    class propertya_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit
    {
        ...
        function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
        {
            ...
            if ($new_fields = propertya_Nav_Menu_Item_Custom_Fields::get_field($item, $depth, $args)) {
...
// Somewhere in config...
add_filter('propertya_menu_item_additional_fields', 'propertya_item_additional_fields');
if (!function_exists('propertya_item_additional_fields')) {
    function propertya_item_additional_fields($fields)
    {
        $fields['some_text'] = array(
            'name' => 'some_text',
            'label' => '*' . esc_html__('If you want to hide column name in mega menu just write {HideMe} in  Navigation Label', 'propertya'),
            'input_type' => 'only_text',
        );
        $fields['color'] = array(
            'name' => 'color',
            'label' => esc_html__('Lebel Color', 'propertya'),
            'container_class' => 'link-color',
            'input_type' => 'color',
        );
        $fields['color_text'] = array(
            'name' => 'color_text',
            'label' => esc_html__('Lebel Text', 'propertya'),
            'container_class' => 'link-color-text',
            'placeholder' => esc_html__('Enter some text', 'propertya'),
            'input_type' => 'text',
        );
        $fields['megamenu_class'] = array(
            'name' => 'is_megamenu_class',
            'label' => esc_html__('This select option is only for 1st level menu', 'propertya'),
            'container_class' => 'megamenu-classes',
            'input_type' => 'select',
        );
        $fields['megamenu'] = array(
            'name' => 'is_megamenu',
            'label' => esc_html__('In parent add bigger value and in 2nd level add small value', 'propertya'),
            'container_class' => 'megamenu-checkbox',
            'input_type' => 'select',
        );
        $fields['menu_icons'] = array(
            'name' => 'menu_icons',
            'label' => esc_html__('Menu Icons (Font Awesome Icons). ', 'propertya') . 'http://fontawesome.io/icons/',
        ...
        $fields['menu_open_side'] = array(
            'name' => 'menu_open_side',
            'label' => esc_html__('Select Options - For simple menu only', 'propertya'),
...
if (!function_exists('propertya_navcustom_callback')) {
    function propertya_navcustom_callback($menu_html = '', $theme_location = '')
    {
          return $menu_html;
    }
}