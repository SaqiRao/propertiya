...
function show_error(error){
   switch(error.code) {
	...
	$(document.body).on('click', '.fetch_result', function () {
		...
		$.post(get_strings.ajax_url, {action: 'prop_agency_search', collect_data: $("form#myagencies_search").serialize(), page_no: page_no,sort_by: sort_by}).done(function (response)
        {
	...
	$('button[name=agencies_search]').on('click', function () {
		...
		$("#listing_ajax_pagination").html('');
		var sort_by = $("#sort-by").val();
        $.post(get_strings.ajax_url, {action: 'prop_agency_search', collect_data: $("form#myagencies_search").serialize(),sort_by: sort_by}).done(function (response)
        {
           $('.fb-like-animation').hide();
           if (true === response.success) 
		   {
	...
	/*Reset All Filters*/
		...
		$(".my-filer-tags").html('');
		$("#listing_ajax_pagination").html('');
		$.post(get_strings.ajax_url, {action: 'prop_agency_search', collect_data: $("form#myagencies_search").serialize()}).done(function (response)
        {
           $('.fb-like-animation').hide();
           if (true === response.success) 
		   {
    ...
    compression: true,
            ...
            source: {
                ajax: {type: "GET", url: get_strings.ajax_url, data: {q: '{{query}}', action: 'prop_agency_search_suggestions' }},
            },
        });
    }
