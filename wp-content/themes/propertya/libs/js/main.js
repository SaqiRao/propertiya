...
function calculateMortgage() {
    ...
    var survey = condo == 0 ? 300 : 0;
    if (GLOBAL_SURVEY) {
        survey = GLOBAL_SURVEY;
    }
    $('#survey').val(numeral(survey).format('0.00'));

    var stateTax = GLOBAL_STATE_TAX ? GLOBAL_STATE_TAX : lessTotalLoanAmount * STATE_TAX;
    $('#stateTax').val(numeral(parseInt(stateTax)).format('0,0.00'));
    var intangibleTax = GLOBAL_INTANGIBLE_TAX ? GLOBAL_INTANGIBLE_TAX : lessTotalLoanAmount * INTANGIBLE_TAX;
    $('#intangibleTax').val(numeral(parseInt(intangibleTax)).format('0,0.00'));
    var ownerTitleInsurance = GLOBAL_OWNER_TITLE_INSURANCE ? GLOBAL_OWNER_TITLE_INSURANCE : ownerTitleInsuranceFunc(homePrice);
    $('#ownerTitleInsurance').val(numeral(parseInt(ownerTitleInsurance)).format('0,0.00'));

    var approxCostClosingFees = getFloat('underwritingFee') + getFloat('originationFee') + getFloat('creditReport');
    approxCostClosingFees += getFloat('appraisals') + getFloat('floodLifeLoanFee') + getFloat('taxServiceFee');
    approxCostClosingFees += getFloat('lenderTitleInsurance') + getFloat('ownerTitleInsurance') + getFloat('closingEscrow');
    approxCostClosingFees += getFloat('recordingFeeMRTG') + getFloat('recordingFeeReleases') + getFloat('stateTax') + getFloat('intangibleTax') + getFloat('survey');
    //$('#approxCostClosingFees').val(numeral(parseInt(approxCostClosingFees)).format('0,0.00'));
    $('#approxCostClosingFees').html(numeral(parseInt(approxCostClosingFees)).format('0,0.00'));

    var interestEstimatedPerDay = GLOBAL_INTEREST_ESTIMATED_PER_DAY ? GLOBAL_INTEREST_ESTIMATED_PER_DAY : parseFloat((lessTotalLoanAmount*mortgageInterest/360).toFixed(6));
    $('#interestEstimatedPerDay').val(numeral(parseFloat(interestEstimatedPerDay)).format('0,0.000000'));
    var interestEstimated = GLOBAL_INTEREST_ESTIMATED ? GLOBAL_INTEREST_ESTIMATED : parseFloat((interestEstimatedPerDay*getInt('interestEstimatedDays')).toFixed(2));
    $('#interestEstimated').val(numeral(parseFloat(interestEstimated)).format('0,0.00'));
    var realEstateTaxEscrow = GLOBAL_REAL_ESTATE_TAX_ESCROW ? GLOBAL_REAL_ESTATE_TAX_ESCROW : propertyTax/12 * getInt('realEstateTaxEscrowMonths');
    $('#realEstateTaxEscrow').val(numeral(parseFloat(realEstateTaxEscrow)).format('0,0.00'));
    var insuranceEscrow = GLOBAL_INSURANCE_ESCROW ? GLOBAL_INSURANCE_ESCROW : homeInsurance/12 * getInt('insuranceEscrowMonths');
    $('#insuranceEscrow').val(numeral(parseFloat(insuranceEscrow)).format('0,0.00'));

    var approxCostPrepaid = getFloat('interestEstimated') + getFloat('realEstateTaxEscrow') + getFloat('insuranceEscrow');
    //$('#approxCostPrepaid').val(numeral(parseFloat(approxCostPrepaid)).format('0,0.00'));
    $('#approxCostPrepaid').html(numeral(parseFloat(approxCostPrepaid)).format('0,0.00'));


    var plusApproxClosingFees = GLOBAL_PLUS_APPROX_CLOSING_FEES ? GLOBAL_PLUS_APPROX_CLOSING_FEES : getFloat('approxCostClosingFees');
    $('#plusApproxClosingFees').val(numeral(plusApproxClosingFees).format('0,0.00'));
    var plusApproxPrepaid = GLOBAL_PLUS_APPROX_PREPAID ? GLOBAL_PLUS_APPROX_PREPAID : getFloat('approxCostPrepaid');
    $('#plusApproxPrepaid').val(numeral(plusApproxPrepaid).format('0,0.00'));


    var totalApproxFunds = getFloat('purchasePrice') - getFloat('lessEarnestMoney') - getFloat('lessTotalLoanAmount') - getFloat('lessSecondaryFinancingAmount') + getFloat('plusApproxClosingFees') + getFloat('plusApproxPrepaid') - getFloat('lessCostPdBy') - getFloat('lessItemsPaidOutside');
    $('#totalApproxFunds').html(numeral(parseFloat(totalApproxFunds)).format('0,0.00'));
    $('#totalApprox').html(numeral(parseFloat(totalApproxFunds)).format('0,0.00'));
