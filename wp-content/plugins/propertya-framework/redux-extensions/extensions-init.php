...
Redux::set_extensions( 'propertya_options', dirname( __FILE__ ) . '/extensions/' );
...
if ( !function_exists( 'propertya_framework_description_text' ) ) {
    function propertya_framework_description_text( $description ) {
        //get ser details
        ...
        $message = '<p>'. esc_html__( 'Best if used on new WordPress install & this theme requires PHP version 7.0+', 'propertya-framework' ) .'</p>';
        $message .= '<p>'. esc_html__( 'Images are for demo purpose only.', 'propertya-framework' ) .'</p>';
        $message .= '
        <h3>Server Requirements</h3>
		<div class="theme-server-details">
    ...
    add_filter( 'wbc_importer_description', 'propertya_framework_description_text', 10 );
}