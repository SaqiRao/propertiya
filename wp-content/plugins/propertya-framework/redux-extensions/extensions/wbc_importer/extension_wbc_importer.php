...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
	...
	if ( !function_exists( 'wbc_after_theme_options' ) )
	{
		...
		function wbc_after_theme_options( $demo_active_import , $demo_data_directory_path )
		{
			...
			if( $current_demo == 'Propertya')
			{
					$home = get_page_by_title('Home 3');
					...
					//Menu
					$top_menu = get_term_by( 'name', 'Propertya Menu', 'nav_menu' );
            ...
            if( $current_demo == 'Propertya RTL')
			{
                    $home = get_page_by_title('الصفحة الرئيسية 3');
					update_option( 'page_on_front', $home->ID );
					update_option( 'show_on_front', 'page' );
					
					// Blog Page
					$blog   = get_page_by_title( 'مدونة' );
					update_option( 'page_for_posts', $blog->ID );
					//Menu
					$top_menu = get_term_by( 'name', 'Propertya Menu', 'nav_menu' );
					if ( isset( $top_menu->term_id ) ) {
