...
//Load More Results
add_action( 'wp_ajax_prop_loadmore_agents', 'propertya_framework_load_moreagents' );
add_action( 'wp_ajax_nopriv_prop_loadmore_agents', 'propertya_framework_load_moreagents' );
if (!function_exists ( 'propertya_framework_load_moreagents' ))
{
	function propertya_framework_load_moreagents()
	{
		...
		if ($results->have_posts())
		{
			...
			while ($results->have_posts())
			{
				$results->the_post();
				$agent_id = get_the_ID();
				$fetch_output .= $layout_type->$function($agent_id,'3');
			}
			wp_reset_postdata();
